...
把 `dsh-session-monitor` 作为 `kevenliu3/dhs-plugins` 大仓的一个**子包**发布到 DSH 插件市场（`dshmarket`）。
...
- 子包路径：`dsh-session-monitor/`
- npm 包（scoped）：`@kevenliu3/dsh-session-monitor`
...
cp -r "/home/fj/Documents/05 skill_agent_plugin/02 dsh-plugins/dsh-session-monitor" .
cat > .gitignore <<'EOF'
node_modules/
*.log
EOF
...
git init
git add .
git commit -m "chore: add dsh-session-monitor"
git branch -M main
...
npm pkg set name=@kevenliu3/dsh-session-monitor
```

> 改 name 后，本地 web profile 里以裸名 `dsh-session-monitor` 安装的 link 会失配。
> 若本地还在用，同步改 `~/.dsh/profiles/web/package.json` 的依赖名为 `@kevenliu3/dsh-session-monitor`，
...
本目录的 `kevenliu3__dhs-plugins--dsh-session-monitor.yml` 已按 monorepo 子包格式写好。提交流程：

```sh
gh repo fork awesome-dsh-plugin/awesome-dsh-plugin --clone
cd awesome-dsh-plugin
cp "/home/fj/Documents/05 skill_agent_plugin/02 dsh-plugins/dsh-session-monitor/kevenliu3__dhs-plugins--dsh-session-monitor.yml" \
   "data/plugins/kevenliu3__dhs-plugins--dsh-session-monitor.yml"
npm ci
node scripts/generate-readme.mjs    # 重新生成两份 README
git checkout -b add/dsh-session-monitor
git add data/plugins README.md README.zh.md
git commit -m "Add dsh-session-monitor"
git push -u origin add/dsh-session-monitor
gh pr create --repo awesome-dsh-plugin/awesome-dsh-plugin --title "Add dsh-session-monitor"
```
...
| 文件 | 用途 |
| --- | --- |
| `kevenliu3__dhs-plugins--dsh-session-monitor.yml` | registry 条目（PR 时放到 `data/plugins/` 下同名文件） |
